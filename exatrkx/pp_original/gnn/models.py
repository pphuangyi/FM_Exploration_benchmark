...
class InteractionGNN(nn.Module):
    ...
    def forward(self, nodes, start_index, end_index):
        ...
        # classification
        clf_input = torch.hstack([nodes[start_index], nodes[end_index], edges])
        return self.classifier(clf_input).squeeze(-1)
...
def assemble_gnn(model_config):
    ...
    classifier_config.update({'input_features': 3 * hidden_features})
    ...
    classifier = MLP(**classifier_config)
