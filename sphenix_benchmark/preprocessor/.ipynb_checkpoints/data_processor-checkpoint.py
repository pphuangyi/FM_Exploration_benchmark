...
class Processor:
    """
    Processing input and construct queries
    """
    def __init__(self,
        ...
        self.num_input_neighbors   = num_input_neighbors
    ...
    def __call__(self, points, track_ids, batch, return_stat=False):
        ...
        knn_head, knn_tail, knn_labels = \
            self.__get_knn_queries(points_hep, track_ids, batch)
        ...
        random_head, random_tail, random_labels \
            = self.__get_random_queries(points_hep, track_ids, batch)
    ...
    def __get_inputs(self, points, batch):
        """
        Since sphenix_fm TPC data only have (x, y, z, Energy),
        we cannot augment the hits with features provided by the
        detector cells as ExaTkx did. So we augment the hit with
        several closest neighbors.
        """
        edge_index = knn(x       = points,
                         y       = points,
                         batch_x = batch,
                         batch_y = batch,
                         k       = self.num_input_neighbors)

        head, tail = edge_index
        _, sorting_indices = head.sort()
        tail = tail[sorting_indices]

        assert len(tail) == self.num_input_neighbors * len(points)

        return points[tail].reshape(points.size(0), -1)
